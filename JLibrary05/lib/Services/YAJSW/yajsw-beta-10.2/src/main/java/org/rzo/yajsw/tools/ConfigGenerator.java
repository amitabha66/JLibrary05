...
public class ConfigGenerator
{
	...
	private static boolean isJavaCmd(Process p)
	{
		...
		try
		{
			...
			VmIdentifier vmidentifier = new VmIdentifier("" + pid);
	...
	private static void createJavaConfigFile(Process p, File input, File output)
	{
		...
		try
		{
			...
			/*
			MonitoredHost monitoredhost = MonitoredHost.getMonitoredHost("//localhost");
			VmIdentifier vmidentifier = new VmIdentifier("" + p.getPid());
			...
			*/
			...
			if (!isNotNullEmpty(mainClass))
			{
				System.out.println("could not retrieve main class of java application -> abort");
				return;
			}
			mainClass = confString(mainClass);
			if (mainClass.endsWith(".jar"))
				conf.setProperty("wrapper.java.app.jar", relativeString(mainClass, workingDir));
			else
...
/*
			...
			if (isNotNullEmpty(jvm))
			{
				jvm = confString(jvm);
				conf.setProperty("wrapper.java.command", jvm);
			}
			*/
			conf.setProperty("wrapper.java.command", parsedCmd.getJava());
			/*String classpath = ((StringMonitor) vm.findByName("java.property.java.class.path")).stringValue();
			if (isNotNullEmpty(classpath))
			{
				...
				{
					conf.setProperty("wrapper.java.classpath." + i, file);
					i++;
				}
			}
			*/
			...
			if (classpathList == null || classpathList.isEmpty() && parsedCmd.getJar() == null)
			{
			...
			// TODO really parse the cmd
			String mainArgs = cmd.substring(cmd.indexOf(" "+mainClass+" ")+mainClass.length()+2);
			if (isNotNullEmpty(mainArgs))
			{
				List args = splitArgs(mainArgs);
				int i = 1;
...
/*
			// bug in MonitoredVMUtil '"-Xd=a a"' returns '-Xd=a a'
			//String jvmArgs = MonitoredVmUtil.jvmArgs(vm);
			// TODO really parse the cmd
			String jvmArgs = cmd.substring(jvm.length(), cmd.indexOf(" "+mainClass+" "));
			if (cmd.startsWith("\""))
				jvmArgs = jvmArgs.substring(1);
			jvmArgs = jvmArgs.replace(classpath, "");
			jvmArgs = jvmArgs.replace(" -classpath ", "");
			jvmArgs = jvmArgs.replace(" -cp ", "");
			
			if (isNotNullEmpty(jvmArgs))
			{
				List args = splitArgs(jvmArgs);
				int i = 1;
				for (Iterator it=args.iterator(); it.hasNext(); )
				{
				String arg = (String) it.next();
...
*/
			...
			String account = p.getUser();
			if (account != null && !"".equals(account))
				conf.setProperty("wrapper.app.account", account);
	...
	{		
		...
		String cp = (String) p.getEnvironment().get("CLASSPATH");
		if (cp == null)
			return null;
