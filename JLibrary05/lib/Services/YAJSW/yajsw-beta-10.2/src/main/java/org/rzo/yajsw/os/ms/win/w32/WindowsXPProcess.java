...
public class WindowsXPProcess extends AbstractProcess
{
	...
	public interface MyKernel32 extends Kernel32
	{
		...
		boolean CreateProcessW(WString lpApplicationName, WString lpCommandLine, Structure lpProcessAttributes, Structure lpThreadAttributes,
				boolean bInheritHandles, int dwCreationFlags, Structure lpEnvironment, WString lpCurrentDirectory, Structure lpStartupInfo,
				Structure lpProcessInformation);
		 ...
		 * @return true, if successful
		 */
		boolean GetProcessAffinityMask(Pointer hProcess, IntByReference lpProcessAffinityMask, IntByReference lpSystemAffinityMask);
		 ...
		 * @return true, if successful
		 */
		boolean SetProcessAffinityMask(Pointer hProcess, int dwProcessAffinityMask);
		 ...
		 * @return true, if successful
		 */
		boolean GetProcessTimes(Pointer hProcess, LongByReference lpCreationTime, LongByReference lpExitTime, LongByReference lpKernelTime,
  ...
  __in      SIZE_T dwLength
	...
	public interface Ntdll extends StdCallLibrary
	{
		 ...
		 * @return the int
		 */
		int ZwQueryInformationProcess(Pointer ProcessHandle, int ProcessInformationClass, Pointer ProcessInformation, int ProcessInformationLength,
	...
	public interface MyAdvapi extends Advapi32
	{
		 ...
		 */
		boolean OpenProcessToken(com.sun.jna.examples.win32.W32API.HANDLE ProcessHandle, int DesiredAccess, PointerByReference TokenHandle);
	...
	public static Process getProcess(int pid)
	{
		...
		Pointer hProcess = MyKernel32.INSTANCE.OpenProcess(MyKernel32.PROCESS_ALL_ACCESS, false, pid);
		if (hProcess == null)
		...
		if (MyAdvapi.INSTANCE.OpenProcessToken(hp, MyAdvapi.TOKEN_READ, hToken))
		{
			...
			if (result._user == null)
				System.out.println(MyKernel32.INSTANCE.GetLastError());
	...
	private boolean setPrivilege(Pointer hToken, String lpszPrivilege, boolean bEnablePrivilege)
	{
		...
		if (!result)
		{
			log("error setting privliges " + Integer.toHexString(MyKernel32.INSTANCE.GetLastError()));
			log("error setting privliges " + MyKernel32.INSTANCE.GetLastError());
		}
		...
		try
		{
			...
			if (_pipeStreams)
			{
				...
				if (!MyKernel32.INSTANCE.SetHandleInformation(inWrite.getValue(), MyKernel32.HANDLE_FLAG_INHERIT, 0)
						...
						// for some unknown reason: if we add the following lines we do not get "operation on non socket" error in mina 
						|| !MyKernel32.INSTANCE.SetHandleInformation(inWrite.getValue(), MyKernel32.HANDLE_FLAG_PROTECT_FROM_CLOSE, MyKernel32.HANDLE_FLAG_PROTECT_FROM_CLOSE)
						|| !MyKernel32.INSTANCE.SetHandleInformation(outRead.getValue(), MyKernel32.HANDLE_FLAG_PROTECT_FROM_CLOSE, MyKernel32.HANDLE_FLAG_PROTECT_FROM_CLOSE)
						|| !MyKernel32.INSTANCE.SetHandleInformation(errRead.getValue(), MyKernel32.HANDLE_FLAG_PROTECT_FROM_CLOSE, MyKernel32.HANDLE_FLAG_PROTECT_FROM_CLOSE))						
			...
			{
				result = MyKernel32.INSTANCE.CreateProcessW(null, cmd, null, null, _pipeStreams, creationFlag, null, wDir, _startupInfo,
						_processInformation);
			}
			else
			{
	...
	private int getExitCodeInternal()
	{
		...
		try
		{
			// if server overloaded windows may need some time to set the exit code.
			Thread.sleep(100);
		}
		...
		// System.out.println("get exit code internal " + result + " " + code.getValue());
		if (result)
		{
			//log("GetExitCodeProcess code "+code.getValue());
			return code.getValue();
		}
		else
		{
			//log("GetExitCodeProcess "+MyKernel32.INSTANCE.GetLastError());
			return -3;
		}
	}
	...
	public boolean kill(int code)
	{
		...
		int i = 0;
		if (_processInformation != null && _processInformation.hProcess != Pointer.NULL)
		while (!result && i < 10)
		{
			if (_processInformation != null && _processInformation.hProcess != Pointer.NULL)
			{
			...
			if (!result)
			{
				log("kill failed: " + _pid+ " error "+ MyKernel32.INSTANCE.GetLastError());
				i++;
				try
				{
					Thread.sleep(500);
	...
	public boolean isRunning()
	{
		...
		return result;
		/*
		Pointer process = MyKernel32.INSTANCE.OpenProcess(MyKernel32.PROCESS_QUERY_INFORMATION, false, _pid);
	...
	boolean readVirtualMemoryToStructure(Pointer baseAddress, Structure goal)
	{
		...
		int ret = Ntdll.INSTANCE.ZwReadVirtualMemory(_processInformation.hProcess, baseAddress, goal.getPointer(), size, null);
		if (ret != 0)
	...
	boolean readVirtualMemoryToMemory(Pointer baseAddress, Memory goal)
	{
		...
		int ret = Ntdll.INSTANCE.ZwReadVirtualMemory(_processInformation.hProcess, baseAddress, goal, size, null);
		if (ret != 0)
			log("pid " + _pid + " ZwReadVirtualMemory returns " + Integer.toHexString(ret));
	...
	String getCommandLineInternal()
	{
		...
		Pointer hProcess = _processInformation.hProcess;
		int size = pbi.size();
					...
					if (peb.ProcessParameters != null)
					{
						...
						if (readVirtualMemoryToStructure(peb.ProcessParameters, userParams))
						{
							...
							if (userParams.Environment != null)
							{
								...
								int ll = MyKernel32.INSTANCE.VirtualQueryEx( hProcess, userParams.Environment,memInfo.getPointer(),memInfo.size());
								memInfo.read();
								if (ll == 0)
								{
			...
			if (pbi.PebBaseAddress != null)
			{
				...
				if (readVirtualMemoryToStructure(pbi.PebBaseAddress, peb))
					if (peb.ProcessParameters != null)
					{
						...
						{
							//System.out.println("MaximumLength " + userParams.CommandLine.MaximumLength);
							//System.out.println("Length " + userParams.CommandLine.Length);
	...
	public static void main(String[] args)
	{
		...
		/*
		WindowsXPProcess p = new WindowsXPProcess();
		p.setCommand("notepad");
		p.setUser("test\\yajsw");
		p.setPassword("yajsw");
		p.start();
		*/
		getProcess(3332);
