...
public abstract class AbstractWrappedProcess implements WrappedProcess, Constants, AbstractWrappedProcessMBean
{
	...
	{	
		...
		{
				_config.addProperty("wrapper.jmx", true);
				_trayIconMessages = new TrayIconProxy();
		}
		
		if (_config.getBoolean("wrapper.tray", false) && _trayIcon == null && !isService())
		{
			registerMBean();
			_trayIcon = WrapperTrayIconFactory.startTrayIcon(_config);
		}
		else if (_config.getBoolean("wrapper.jmx", false))
			registerMBean();
	...
	private void configClusterScript(String clusterScript)
	{
		if (clusterScript != null && !"".equals(clusterScript))
		{
			...
			final Script script = ScriptFactory.createScript(clusterScript, "", this, args, getWrapperLogger());
			if (script == null)
				return;
			try
			{
	...
	{
		// on windows services: shutdown is handeled by the onStop method not by the hook
	...
	private void configStateChangeListeners()
	{
		...
		for (Iterator it = listeners; it.hasNext(); )
		{
			...
			final Script script =  ScriptFactory.createScript(value, state, this, args, getWrapperLogger());
			...
			addStateChangeListener(STATE_IDLE, new StateChangeListener()
			{
				public void stateChange(int newState, int oldState)
				{
					...
					executor.execute(new Runnable()
					{
						...
						public void run()
						{
							// if this is a service: do not exit here so that we can inform the service controller
							if (!isService())
								System.exit(0);
							else
							{
		...
		try
		{
			...
			if (server != null)
			{
				...
				if (port >0)
				{
					...
					if (_config.getString("wrapper.jmx.rmi.user", null) != null)
					{
						...
						JMXAuthenticator authenticator = new JMXAuthenticator()
						{
							...
							public Subject authenticate(Object credentials)
							{
							      ...
							      if (!myUser.equals(user)) throw new SecurityException("Unknown user " + user);
							      if (!myPassword.equals(password)) throw new SecurityException("Bad password");
					...
					JMXConnectorServer cs =  JMXConnectorServerFactory.newJMXConnectorServer(url, environment, server); 
					cs.start(); 
				}
			}
			else
	...
	public synchronized void startInternal()
	{
		...
		if (_controller != null)
		{
			...
			{
				System.out.println("could not start controller -> abort");
	...
	private void updateAppLoggerName()
	{
		if (_appLogger == null)
			return;
		_appLoggerName = getAppPid() + "/" + _restartCount;
	...
	{
		String wFormat = _config.getString("wrapper.log.format", "LPTM");
	...
	{
		String wFormat = _config.getString("wrapper.console.format", "LPTM");
		return getFormatter(wFormat);
	}
	...
	protected Map getMissingTriggerActions()
	{
		...
		for (Iterator it = _config.getKeys("wrapper.filter.missing.trigger"); it.hasNext();)
		{
			...
			{
				System.out.println("check parameters for " +tKey);
				continue;
			}
			String tValue = (String) lValue.get(0);
			// commons configuration does no accept <space>, 1, 1  as valid list -> using * instead of space
			if ("*".equals(tValue))
				tValue = "";
			int countValue = -1;
			try
			...
			Object script = getTriggerScript(sValue, tKey.substring(tKey.lastIndexOf('.')), strArgs);
			if (action != null || script != null)
			{
				result.put(tValue, 
				new MissingTriggerAction(executor, periodValue, countValue, new TriggerAction[]{(TriggerAction)script, (TriggerAction)action}) );
			}
		}
		return result;
	}
	...
	protected Map getTriggerActions()
	{
		...
		for (Iterator it = _config.getKeys("wrapper.filter.trigger"); it.hasNext();)
		{
			...
			if (action != null && script != null)
			{
				result.put(tValue, Arrays.asList(new Object[]
				{ script, action }));
			}
			else if (action != null)
				result.put(tValue, action);
			else if (script != null)
				result.put(tValue, script);
			else
				result.put(aKey, "RESTART");
		}
		return result;
	}
	 ...
	 */
	private Object getTriggerScript(String script, String key, String[] args)
	{
		final Script s = ScriptFactory.createScript(script, key, this, args, getWrapperLogger());
		if (s == null)
		{
		...
		return new TriggerAction()
		{
			public void execute(final String line)
			{
				scriptExecutor.execute(new Runnable()
				{
					...
					public void run()
					{
						AbstractWrappedProcess.this.setTriggerLine(line);
						// s.execute(id, stringState(_state), "" + _restarts, ""
						// + getPid(), "" + getExitCode(), line,
						// AbstractWrappedProcess.this);
	...
	public void restartByTimer()
	{
		...
		{
			if (_debug)
				getWrapperLogger().throwing(this.getClass().getName(), "restart", e);
			Thread.currentThread().interrupt();
		}
	...
	protected boolean saveLockFile()
	{
		if (_lock != null)
			return true;
		String file = _config.getString("wrapper.lockfile");
	...
	public Logger getWrapperLogger()
	{
		...
		((MyLogger) _wrapperLogger).setName(_wrapperLoggerName);
	...
	{
		System.out.println("restart internal "+getStringState());
	...
	private Script getRestartDelayScript()
	{
		...
		if (script != null)
		{
			...
			_restartDelayScript = ScriptFactory.createScript(script, "", this, args, getWrapperLogger());
		}
		return _restartDelayScript;
	}
	...
	protected class Gobler implements Runnable
	{
		...
		public void setDrain(boolean drain)
		{
			if (drain && !_drain)
			{
				_drainBuffer = new CircularBuffer(2048, false);
		...
		public Gobler(InputStream stream, Logger log, Map events, Map eventsRegex, Map missingEvents, Map missingEventsRegex, String name, int pid)
		{
			...
			if (eventsRegex != null)
			{
				...
				for (Iterator it = eventsRegex.keySet().iterator(); it.hasNext(); i++)
				{
					...
					{
						System.out.println();
						System.out.println("error in regular expression " + s + ": " + ex);
					}
				}
			}
		...
		public void run()
		{
				...
				while ((line = br.readLine()) != null)
				{
					...
					if (_actionTriggers != null)
						for (int i = 0; i < _actionTriggers.length; i++)
						{
							...
							{
								Object obj = (TriggerAction) _actions.get(_actionTriggers[i]);
					...
					if (_missingActionTriggers != null)
						for (int i = 0; i < _missingActionTriggers.length; i++)
						{
							if ("".equals(_missingActionTriggers[i]) || line.contains(_missingActionTriggers[i]))
							{
								...
								{
									TriggerAction action = (TriggerAction) obj;
									//getWrapperLogger().info("Trigger found: " + _missingActionTriggers[i]);
									action.execute(line);
								}
								//break;
							}
					...
					if (_missingActionTriggersRegex != null)
						for (int i = 0; i < _missingActionTriggersRegex.length; i++)
						{
							if (_missingActionTriggersRegex[i].contains(line))
							{
								...
								{
									TriggerAction action = (TriggerAction) obj;
									//getWrapperLogger().info("Trigger found: " + _missingActionTriggersRegex[i]);
									action.execute(line);
								}
								//break;
							}
			...
			{
				// ioe.printStackTrace();
				_goblerLog.info("gobler execption "+_name+ " "+ ioe.getMessage());
				if (AbstractWrappedProcess.this._osProcess != null && _pid == AbstractWrappedProcess.this._osProcess.getPid() && AbstractWrappedProcess.this._osProcess.isRunning())
					AbstractWrappedProcess.this.executor.execute(new Runnable()
					{
						public void run()
						{
							_goblerLog.info("yajsw panicking: gobler stopped but process is still running -> restart process");
							if (AbstractWrappedProcess.this._state != STATE_RESTART_START && AbstractWrappedProcess.this._state != STATE_RESTART && AbstractWrappedProcess.this._state != STATE_RESTART_STOP && AbstractWrappedProcess.this._state != STATE_RESTART_WAIT)
	...
	{
		if (_osProcess != null)
			_exitCode = _osProcess.getExitCode();
	}
	...
	{
		System.out.println("set proerty "+key + " " + value);
