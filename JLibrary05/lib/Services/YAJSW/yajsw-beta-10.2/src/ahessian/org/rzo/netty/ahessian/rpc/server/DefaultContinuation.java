 ...
 */
class DefaultContinuation implements Continuation
{
	 ...
	 DefaultContinuation(HessianRPCCallMessage message, ContinuationService service, Session session)
	{
		...
		_headers.put("completed", Boolean.FALSE);
		_session = session;
	}
	...
	{
		if (_completed)
			_headers.put("completed", Boolean.TRUE);
		_service.writeResult(new HessianRPCReplyMessage(result, fault, _headers, null));
	}
